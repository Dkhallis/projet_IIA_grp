...
function App() {
  ...
  async function login(event) {
    ...
    if (role === 'Administrateur') {
      ...
      notify('La connexion du personnel n’est pas disponible.')
      return
    }
    setPage('overview')
  }
  ...
  const pageTitle = page === 'overview' ? `Bonjour ${user.name.split(' ')[0]} 👋` : page === 'reports' ? (staff ? 'Centre des signalements' : 'Mes signalements') : page === 'all-reports' ? 'Tous les signalements' : page === 'messagerie' ? 'Messagerie' : page === 'analytics' ? 'Statistiques' : page === 'history' ? 'Historique des actions' : page === 'users' ? 'Utilisateurs' : 'Configuration'
  return <div className="app-shell"><Sidebar user={user} page={page} setPage={setPage} logout={() => setUser(null)} /><main className="main-content"><header className="topbar"><div><p className="topbar-kicker">CampusHelp / {user.role}</p><h1>{pageTitle}</h1></div><div className="topbar-actions"><button className="icon-button" onClick={() => notify('Vous avez 3 notifications non lues')} aria-label="Notifications">♧<span>3</span></button><div className="user-chip"><span className="avatar">{user.name.split(' ').map((part) => part[0]).join('')}</span><span><b>{user.name}</b><small>{user.role}</small></span></div></div></header>{notice && <div className="toast">✓ {notice}</div>}{page === 'overview' && <Overview user={user} reports={reports} setPage={setPage} open={setSelected} create={() => setModal(true)} />}{page === 'reports' && <Reports reports={staff ? visibleReports : myReports} all={staff ? reports : reports.filter((item) => item.author === user.name)} staff={staff} query={query} setQuery={setQuery} status={status} setStatus={setStatus} category={category} setCategory={setCategory} open={setSelected} create={() => setModal(true)} update={updateReport} kicker={staff ? 'Pilotage opérationnel' : 'Votre historique'} heading={staff ? 'Toutes les demandes du campus' : 'Vos signalements'} showCreate={!staff} />}{page === 'all-reports' && <Reports reports={visibleReports} all={reports} staff={staff} query={query} setQuery={setQuery} status={status} setStatus={setStatus} category={category} setCategory={setCategory} open={setSelected} create={() => setModal(true)} update={updateReport} kicker="Vue communautaire" heading="Tous les signalements" showCreate={false} />}{page === 'messagerie' && <Messagerie user={user} conversations={conversations} send={sendMessage} />}{page === 'analytics' && <Analytics reports={reports} />}{page === 'history' && <History />}{page === 'users' && <Users notify={notify} />}{page === 'settings' && <Settings setPage={setPage} notify={notify} />}{selected && <Drawer report={reports.find((item) => item.id === selected)} close={() => setSelected(null)} staff={staff} update={updateReport} user={user} />}{modal && <CreateModal close={() => setModal(false)} submit={createReport} />}</main></div>
}
